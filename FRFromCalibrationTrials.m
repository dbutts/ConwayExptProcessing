...
function [meanFR, FR_std_SU] = FRFromCalibrationTrials(filenameP, dirpath, ks_path)
...
for ks_batch = 1:num_ks_batch
    ...
    try
        ...
        meanFR{ks_batch} = single(zeros(numel(allUnit_clusterIDs{ks_batch}),  1));

        for unit = 1: numel(allUnit_clusterIDs{ks_batch})
            unitID = allUnit_clusterIDs{ks_batch}(unit);

            meanFR{ks_batch}(unit,:) = sum(spk_clusters==unitID);
            % compute FR std

            N = histcounts(spk_times(spk_clusters==unitID), 'binedges', 0:dt:t_plexon(end));
            FRs = N/dt;

            FR_std{ks_batch}(unit,:) = std(FRs);
    ...
    FRSU{ks_batch} = meanFR{ks_batch}(1:nSU{ks_batch},:);
    FRMU{ks_batch} = meanFR{ks_batch}(nSU{ks_batch}+1:nSU{ks_batch}+nMU{ks_batch},:);
    %%%%
...
meanFR = FRSU/t_plexon(end);
FR_std = vertcat(FR_std{:});
FR_std_SU = FR_std(1:nSU);
