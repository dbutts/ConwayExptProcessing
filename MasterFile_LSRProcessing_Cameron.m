...
% can be loaded or its outputs (in memory can be used directlyf
...
target_SUs = [9];
lag = 5;
get_hartleys(data_hartleys, target_SUs, lag);
...
target_SUs = [1:size(data.Robs, 1)]; % Change selection to a specific number of SUs Total number of SUs = size(data.Robs, 1)
apply_ETshifts = 1;
stim_deltas = 0; 
num_lags = 6;
