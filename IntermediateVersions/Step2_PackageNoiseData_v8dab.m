...
if useofflinesorting == 1
	nChans=nSU+nMU;
	spk_ID=[spk_ID_SU;spk_ID_MU];
	spk_ch=[spk_channels_SU+1;spk_channels_MU+1];
else
	nChans=24; %targchans=1:24;
end

targ_stimtype = 8;    %this selects which stimulus to process: 8 looks for cloud stims
targ_ETstimtype = 7;  %this checks which ET stim to process: set to 7 if you presented Clouds as ET stim, 1 if you used 1D bars
...
%%
%ntrls = min([size(ExperimentRecording_mod,1),500]); % only using the first 500
...
cd(stimFilePath)
cur_scale=g_astrctAllParadigms{1, 1}.DualstimScale.Buffer(find(g_astrctAllParadigms{1, 1}.DualstimScale.TimeStamp<ExperimentRecording_mod{2,2},1,'last'));
...
for tt=1:ntrls
    ...
    stimloc(tt,:)=ExperimentRecording_mod{tt, 1}.m_aiStimulusRect;
...
for cc=1:size(binned_SU,2)
    ...
    colormap(gray); xlabel(['Lag ' num2str(curlag)]);
    if curlag==1; ylabel('S          L-M          Lum'); title(['ch#', num2str(spk_ch(cc)), 'spkID' num2str(spk_ID(cc))]); end
    end
%    figtitle(['Probe ' num2str(Robs_probe_ID(cc)) '   Unit ' num2str(cc) ])
