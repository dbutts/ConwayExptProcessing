...
% This code reads in Kofiko, plexon, and Kilosort files and aligns them
% using the Kofiko trials as a base. To use this script, change the paths
% in the first section to the locations of the corresponding files.

%Output: the ExperimentRecording struct, which is N_trials long and
%contains:
% column 1 has all the Kofiko trial information, 
% column 2 contains kofiko onset times
% column 3 contains kofiko paradigm name
% column 4 contains LFP
% column 6 contains detected saccades
% column 7 contains eye position data
% column 9 contains plexon onset times of each trial
% columns 10 and every 3rd column after contains all spike times and
% kilosort info of one unit, aligned to that trial.
clear all

%% 
filenameP = '220722_144441_Jacomo'; %the "base" of the filename

ET_Eyelink=2; % set to 0 for Iscan data; Set to 1 for binocular eye link experiments; Set to 2 for monocular Eyelink Experiments ( 0616 to 0709); set to 3 for dDPI
spk_offset=0; % set to 0 when plexon alignment bug doesn't happen - otherwise, set to plexon offset in seconds, e.g. [58.352];
iso_SUs=[]; 

useofflinesorting = 1; % set to 1 if using kilosort
skipLFP=0; % set to 1 if you want to save space and avoid processing LFPs
...
nChans=24; %number of channels to be used for CSD (24 for our laminar probes

sessionsToProcess = [];
ExperimentFolder = '/media/felix/Internal_1/Data/BevilColor/'; % what is the base directory?
%plxFilePath = [ExperimentFolder filenameP '.pl2']; % where is the plexon file?
plxFilePath = ['/home/felix/NTlab_dataserver4/Conway/raw/' filenameP '.pl2'];

matFilePath = [ExperimentFolder filenameP '/']; % path to the the kofiko trial folder
configFilePath = [ExperimentFolder filenameP '.mat']; % path to the the kofiko configuration file

KSstitched=1; ksFilePath = [ExperimentFolder filenameP '/kilosorting_stitch/']; arraylabel ='full'; % path to kilosort outputs
%KSstitched=0; ksFilePath = [ExperimentFolder filenameP '/kilosorting_laminar/']; arraylabel ='lam';

sessionTimeOffsets = [];                                     
strExperimentPath = [matFilePath '/Analysis/']; % where is processed stuff going to be saved?
if ~exist(strExperimentPath,'dir');
    mkdir(strExperimentPath);
end
filenameK = matFilePath;
load(configFilePath)
%% test for plexon misalignment
%/{
...
g_strctStatistics.preTrialWindow = 0; %trial alignment start (seconds), set to negative values if you want to get any pretrial info for baseline correction stuff
g_strctStatistics.postTrialWindow = 4; %trial end (seconds)
...
%    spk_clustIDs = spk_info.cluster_id; nclusts=length(spk_clustIDs);
    ...
    try
        ...
        try
            ...
            spk_rating_SU = (spk_info.r(spk_labels_SU));
            spk_rating_MU = (spk_info.r(spk_labels_MU));
        end
    end
...
%     end
    ...
    ExperimentRecording{iTrials,9} = ExperimentRecording{iTrials,1}.PlexonOnsetTime; %save Plexon onset time separately ao that we can easily extract it for NWB formatting
