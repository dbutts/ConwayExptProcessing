...
%filenameP = '190412_135949_Jacomo';

% ExperimentFolder = '/home/fellixbartsch/Data/';
% plxFilePath = ['/home/fellixbartsch/Data/' filenameP '.pl2'];
% matFilePath = ['/home/fellixbartsch/Data/' filenameP '/'];
% %strExperimentPath = matFilePath;%pwd;
% load([strExperimentPath,'/FullExperimentRecordingv5.mat'])
...
targ_ETstimtype=7; % 7 for both Cloud; 1 for both 1D bars
targ_stimtype=8;
...
%    test(tt)=ExperimentRecording{tt, 1}.m_bMonkeyFixated;
...
trlonset_diffs = [4; diff(cell2mat(ExperimentRecording(:, 2)))];
...
%         try
%             binned_SU2(cur_trlinds,channel)=histcounts(ExperimentRecording_mod{tt,10+3*(channel-1)}.unit2,edges_hist);
% %         catch
% %             binned_SU2(cur_trlinds,targchans(channel))=0;
%         end 
%         try
%             binned_SU3(cur_trlinds,channel)=histcounts(ExperimentRecording_mod{tt,10+3*(channel-1)}.unit3,edges_hist);
%         end
%         try
%             binned_SU4(cur_trlinds,targchans(channel))=histcounts(ExperimentRecording_mod{tt,10+3*(channel-1)}.unit4,edges_hist);
%         end
    %binned_MUA(cur_trlinds,channel)=histc(ExperimentRecording_mod{tt,12+3*(channel-1)},edges);
    %LFP_trace(channel,cur_trlinds)=splitapply(@mean, ExperimentRecording_mod{tt,4}(channel,:), LFP_labels);
    end
%}
...
%     cur_trial_ET_trace_full(:,1)=([ExperimentRecording_mod{tt,1}.m_afEyeXPositionScreenCoordinates-ExperimentRecording_mod{tt,1}.m_pt2iFixationSpot(1)])./pixelscaf;
%     cur_trial_ET_trace_full(:,2)=([ExperimentRecording_mod{tt,1}.m_afEyeYPositionScreenCoordinates-ExperimentRecording_mod{tt,1}.m_pt2iFixationSpot(2)])./pixelscaf;
    cur_trial_ET_trace_full=[]; cur_trial_ET_trace=[];
%    cur_trial_ETsamples=[0:.001:3.999];
...
if ET_Eyelink==3
    cur_trial_ET_trace_full(:,1)=(ExperimentRecording_mod{tt, 7}.ET_trace(3,:)' - median(ExperimentRecording_mod{tt, 7}.ET_trace(3,:)'))*(g_strctEyeCalib.GainX.Buffer(end)./1000);
    cur_trial_ET_trace_full(:,2)=(ExperimentRecording_mod{tt, 7}.ET_trace(4,:)' - median(ExperimentRecording_mod{tt, 7}.ET_trace(4,:)'))*(g_strctEyeCalib.GainY.Buffer(end)./1000);    
end 
...
%% do same processing of ET data as done for saccade detection
    if ET_Eyelink ==3
        ...
        ET_trace_raw_1khz([1:trlsecs*1000]+trlsecs*1000*(tt-1),3) = ExperimentRecording_mod{tt, 7}.ET_trace(1,:)';
...
switch filenameP    
    case '190705_140023_Jacomo'
         binned_SU(:,1)=binned_SU1(:,1)+ binned_SU1(:,2);
         binned_SU(:,4)=binned_SU2(:,6);
        binned_SU(:,8)=binned_SU1(:,6)+ binned_SU1(:,7)+ binned_SU1(:,8);
        binned_SU(:,9)=binned_SU1(:,6)+ binned_SU1(:,7)+ binned_SU1(:,8)+ binned_SU1(:,9);
        binned_SU(:,10)=binned_SU1(:,10)+ binned_SU1(:,11)+binned_SU1(:,12);
        binned_SU(:,11)=binned_SU2(:,12);
      
        iso_SUs=[1 3 4 5 8 9 11 12 13 14 15 19 21 23 24];
end
%{
%% quick STA test
...
%% for running on packaged data
binned_SU=double(Robs');
%binned_SU=double(Robs_Laminar');

stim=permute(stim,[4 1 2 3]); %stimX=stim;
stim2=(double(reshape(stim,size(stim,1),3*60*60))./127);
...
%{
%% test the simulus transformations
stimtest=zeros(2,3,25,25);
stimtest(1,1,1:10,1:10)=1;
stimtest(1,2,1:10,1:10)=-1;
stimtest(1,3,1:10,1:10)=-1;

stimtest(2,1,11:20,11:20)=1;
stimtest(2,2,11:20,11:20)=-1;
stimtest(2,3,11:20,11:20)=-1;

stimtest2=reshape(stimtest,size(stimtest,1),3*25*25);
%figure; imagesc(stimtest2)
%
cur_STAtest=[1;0.5]'*stimtest2;
%    figure; imagesc(cur_STAtest)
cur_STAtest2=reshape(cur_STAtest,3,25,25);
cur_STAtest3=reshape(permute(cur_STAtest2,[3 2 1]),25,75);
    figure; imagesc(cur_STAtest3)
%}
%{
for cc=1:25
    for curlag=1:5
%    cur_STA=binned_MUA(cur_use_inds+curlag,cc)'*stim2(cur_use_inds,:);
    cur_STA=binned_SU(cur_use_inds+curlag,cc)'*stim2(cur_use_inds,:);
    cur_StA2=reshape(cur_STA,3,25,25);
    cur_StA3=reshape(permute(cur_StA2,[3 2 1]),25,75);
%    cur_StA3=reshape(cur_STA',75,25)';
    curlim=max(abs(cur_StA2(:)));
    cur_StA3(:,[25 50])=curlim;
    subplot(5,1,curlag)
    imagesc((cur_StA3)); caxis([-curlim curlim]); pbaspect([3 1 1])
    colormap(gray); ylabel(['Lag ' num2str(curlag)]);
    if curlag==5; xlabel('Lum             L-M             S'); end
    end
    figtitle(['Unit ' num2str(cc)])
    pause
end
%}
