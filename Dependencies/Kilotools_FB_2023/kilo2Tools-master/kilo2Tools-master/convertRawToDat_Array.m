function [samples, datPath] = convertRawToDat_Array(rawFullPath, opts)
...
if exist(datPath, 'file')
    ...
    if strcmpi(confirmDataDelete, 'Yes')
        ...
        error('Execution stopped due to existing dat file: %s', datPath)
    end
end
...
% 				fidout = fopen(datPath, 'a'); % opening file for appending  
    ...
    %					samples(i, ts) = snip(ts);
            		    ...
            		    disp([spkcOnChanNames{i+opts.ChnOffset + min(opts.curchannels)-1} ' read took ' num2str(durationStore(i)) ...
