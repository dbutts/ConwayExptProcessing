function [samples, datPath] = convertRawToDatv3_Utah(rawFullPath, opts)
...
fprintf('  CONVERSION TO KS-dat file: Using batch size of %d\n', batch_size)
%%%%%%%%
...
switch rawFileType
    ...
    case 'pl2'
        ...
        % a place to store the data that's been read in.
				%%%%%%%%%%% DAB MODIFIED %%%%%%%%%%%%%
        %samples = zeros(nChannels, nSamples, 'int16');
        NBATCH = ceil(nSamples/batch_size);
...
% 				fidout = fopen(datPath, 'a'); % opening file for appending  
				...
				for nn = 1:NBATCH
					...
					if nn < NBATCH
							ts = ((nn-1)*batch_size+1):(nn*batch_size);
					else
							ts = ((nn-1)*batch_size+1):nSamples;
					end

					samples = zeros(nChannels, length(ts), 'int16');

        		    % loop over channels and store in a large array.
					for i = 1:nChannels
            		
            		% tell user which channel is being read.
            	%	disp(['Reading from ' spkcOnChanNames{i+opts.ChnOffset} '.']);
            	%	disp(['Reading from ' ['SPKC' num2str(i+opts.ChnOffset)] '.']);
            		disp(['Reading from ' ['SPKC' sprintf('%03d', (i+opts.ChnOffset))] '.']);
                    ...
                    %snip = adReadWrapper(fullFileName, spkcOnChanNames{i+opts.ChnOffset})';
%					snip = adReadWrapper(fullFileName, ['SPKC' num2str(i+opts.ChnOffset)])';
					snip = adReadWrapper(fullFileName, ['SPKC' sprintf('%03d', (i+opts.ChnOffset))])';
...
%             		disp([spkcOnChanNames{i+opts.ChnOffset} ' read took ' num2str(durationStore(i)) ...
%                 		' seconds. Estimated time to finish reading (min, mean, max): '...
%                 		'(' num2str(minTimeEst) ', ' num2str(meanTimeEst) ', ' ...
%                 		num2str(maxTimeEst) ').']);
            		disp([['SPKC' num2str(i+opts.ChnOffset)] ' read took ' num2str(durationStore(i)) ' seconds. Estimated time to finish reading (min, mean, max): '...
                		'(' num2str(minTimeEst) ', ' num2str(meanTimeEst) ', ' num2str(maxTimeEst) ').']);            		
					end        
					fwrite(fidout, samples, 'int16'); %%% DAB
                    fclose(fidout);  %%% FB

				end      				%%% DAB
				% fclose(fidout);     	%%% DAB, then FB commented out
...
if opts.commonAverageReferencing
    disp('Performing common average subtraction...')
		disp('OOPS THIS WONT WORK -- ABOUT TO CRASH')   % DAB
