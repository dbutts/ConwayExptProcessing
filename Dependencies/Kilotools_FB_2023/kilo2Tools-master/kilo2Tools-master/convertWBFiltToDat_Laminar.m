function [samples, datPath] = convertWBFiltToDat_Laminar(rawFullPath, opts)
...
switch rawFileType
    ...
    case 'pl2'
        ...
        spkcChans   = cellfun(@(x) contains(x, 'FILT_WB'), adChanNames);
...
if opts.commonAverageReferencing==1
...
if opts.commonAverageReferencing==2
    disp('Performing common average subtraction...')
    samplesMean = int16(median(samples));
    figure,
    plot(samplesMean);
    title('the sample''s mean, subtracted from all channels')
    samples = bsxfun(@minus, samples, samplesMean);
