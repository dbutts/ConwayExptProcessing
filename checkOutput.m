...
fullet = exist([outputdir filenameP '_FullExpt_ET.mat'], 'file');
fixInfo = exist([outputdir filenameP '_fixinfo.mat'], 'file');

fprintf('\n\n**CheckOutput()**\n')
disp('All all of the analysis files saved?');
disp([outputdir filenameP '_laminar_CC_ETNA_v09.mat:'])
disp(string(etna))
disp([outputdir filenameP '_FullExpt_ET.mat:'])
disp(string(fullet))
disp([outputdir filenameP '_fixinfo.mat:'])
disp(string(fixInfo))

fprintf('\nDoes fixinfo have the necissary information?')
 % Load metadata from the .mat file
info = whos('-file', [outputdir filenameP '_fixinfo.mat']);
